<?php
  ...
  $texto_log = 'O usuario ' . $usuario_logado . ' acessou os logs do sistema.';
  $result_2 = mysqli_query($conexao, "INSERT into logs values (null, '$id_usuario_logado', '$texto_log', 
    NOW(), '$usuario_logado', '$ip')");
  // fim registrar o log
  
  // opçao para filtar pelo usuário, data inicial e dara final
  $filtro = ' (2 > 1) '; 
  
  // opçoes de filtro e ordenaçao
  $ordenacao = 'data';
  if(isset($_GET["ordem_por"])){
	$ordenacao = tratar_entrada($_GET["ordem_por"]);
	$usuario_filtro = tratar_entrada($_GET["usuario_filtro"]);
	$data_inicial = tratar_entrada($_GET["data_inicial"]);
	$data_final = tratar_entrada($_GET["data_final"]);
	
	if($usuario_filtro != 0){
	  $filtro = " usuario = " . $usuario_filtro . " AND DATE(data) between '" . $data_inicial . "' AND '" . $data_final . "'";	
	}
	else{
	  $filtro = " DATE(data) between '" . $data_inicial . "' AND '" . $data_final . "'"; 	
	}  
  }
  ...
  // fim opçoes de filhtro e ordenaçao
  
  if(isset($_GET["pesquisa"])){
	 $pesquisa_categoria = tratar_entrada(utf8_decode(trim($_GET["pesquisa"]))); 
     $sql_text = ("select id, nome, DATE_FORMAT(data_cadastro, '%d/%m/%Y - %H:%i') as data_cadastro, usuario FROM categorias WHERE " . $filtro . " AND (nome LIKE '%$pesquisa_categoria%' OR descricao LIKE '%$pesquisa_categoria%') order by " . $ordenacao . " " . $ordem_class);
     $sql_text2 = ("select id FROM categorias WHERE " . $filtro . " AND (nome LIKE '%$pesquisa_categoria%' OR descricao LIKE '%$pesquisa_categoria%')");
  }
  else{
	 $sql_text = ("select id, usuario, texto, DATE_FORMAT(data, '%d/%m/%Y - %H:%i') as data_cadastro, nome_usuario, ip from logs WHERE " . $filtro . " order by " . $ordenacao . " " . $ordem_class);
     $sql_text2 = ("select id from logs WHERE " . $filtro);  
  }
...
<head>
  ...
  <title><?= $titulo_aplicacao ?> - Logs do Sistema</title>
...
<div id="layoutSidenav">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        ...
                        <h1 class="mt-4"><i class="bi-file-binary"></i> Logs do Sistema</h1>
                        ...
                        <ol class="breadcrumb mb-4">
                            ...
                            <li class="breadcrumb-item active">Atividades no Sistema</li>
                        </ol>
                        </td>
                        </tr>
                        </table>                     
                        ...
                        <div class="card mb-4">
                            <div class="card-header">
                                <table width="100%">
                                <form id="pesquisa_logs" nome="pesquisa_logs" action="logs.php" method="get">
                                <tr>
                                <td nowrap><i class="bi-file-binary"></i> Atividades do Sistema</td>
                                
                                <td nowrap align="right">
                                
                                <div style="width:230px" class="input-group">
    
    <a title="OpÃ§oes" class="nav-link dropdown-toggle" id="navbarDropdown" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
  ...
  <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
    ...
    <li><a class="dropdown-item" href="logs.php">Limpar Pesquisa e Filtro</a></li>
  </ul>
    <input type="text" name="pesquisa" id="pesquisa" <?= (isset($_GET["pesquisa"]) ? ' value="' . $pesquisa_logs . '"' : '') ?> required class="form-control" placeholder="Pesquisar">
      ...
      <button class="btn btn-secondary" type="submit">
        <i class="fa fa-search"></i>
					   ...
					   if($num_rows2 == 0){
						   ...
						   echo '<b>' . $num_rows2 . '</b> registros encontrados. PÃ¡gina <b>' . $page . '</b> de 
					   ...
					   <?php
                         ...
                         if($resultado_paginacao){
                            while($detalhes = mysqli_fetch_array($resultado_paginacao)){
                               $ordem++;
					           $id = $detalhes["id"];
							   $nome_usuario = utf8_encode($detalhes["nome_usuario"]);
							   $texto = utf8_encode($detalhes["texto"]);
							   ...
							   echo '<td style="vertical-align: middle" nowrap="nowrap" align="left">' . $nome_usuario . '</td>';
							   echo '<td style="vertical-align: middle" nowrap="nowrap" align="left">' . $texto . '</td>';
    ...
    <div class="modal-dialog" style="width: 60%;">
        <div class="modal-content" >
          ...
          <div id="janela_paginacao" class="modal fade" role="dialog" style="width:100%">
            <form name="form_paginacao" action="logs.php" method="get" id="form_paginacao">
            <div class="modal-dialog" style="width: 60%;">
               <div class="modal-content" >
                 ...
                 <div class="mb-3">
                 <label style="padding-left: 5px" class="form-control-label">UsuÃ¡rio<span class="text-danger"> *</span></label>
                 <select name="usuario_filtro" id="usuario_filtro" style="margin-bottom: 10px; margin-top: 10px" class="form-select">
                    <?php
						$result = mysqli_query($conexao, "select id, usuario from usuarios order by usuario");
                        if($result){
	                       echo '<option value="0">Todos</option>';
						   while($detalhes = mysqli_fetch_array($result)){
	                          echo '<option value="' . $detalhes["id"] . '">' . utf8_encode($detalhes["usuario"]) . '</option>';
	                       }
                       }
					?>
                 </select>
                 ...
                 <input type="date" required class="form-control" value="<?= date('Y-m-d') ?>" name="data_inicial" id="data_inicial" placeholder="Escolha a data">
                 </div>
                 
                 <div class="mb-3">
                 <label style="padding-left: 5px" class="form-control-label">Data Final<span class="text-danger"> *</span></label>
                 <input type="date" required class="form-control" value="<?= date('Y-m-d') ?>" name="data_final" id="data_final" placeholder="Escolha a data">
                 </div>
                 ...
                 <div class="mb-3">
                   ...
                   <select class="form-select" id="quant_paginas" name="quant_paginas">
                      ...
                      <?php
					    for($i = 0; $i < count($opcoes_paginacao_logs); $i++){
						   echo '<option value="' . $opcoes_paginacao_logs[$i] . '">' . $opcoes_paginacao_logs[$i] . ' itens por p&aacute;gina</option>';	
						}
					  ?>
                 ...
                 <div class="mb-3">
                   ...
                   <select class="form-select" id="ordem_por" name="ordem_por">
                      ...
                      <?php
					    $nomes_campos = array('nome_usuario', 'texto', 'data', 'ip');
						$titulos_campos = array('UsuÃ¡rio', 'Texto', 'Data', 'IP');
